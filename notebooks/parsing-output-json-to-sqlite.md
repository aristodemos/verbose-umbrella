Yes. The sample JSON has this useful shape:
...
So the loader should mainly populate:
...
For the sample file, the strongest structure is the table data: table headers plus row cells. The script below stores the raw parsed JSON, structural layout, table cells, and then creates one semantic `record` per table row plus one `fact` per non-empty table cell. It is designed around your uploaded sample JSON. 
...
```bash
python import_pdf_json.py int_report.result-multi0.json --db pdf_knowledge_dev.sqlite
```

For your uploaded sample in this environment, the equivalent path would be:

```bash
python import_pdf_json.py /mnt/data/int_report.result-multi0.json --db pdf_knowledge_dev.sqlite
```

One important design choice in this script: it does **not** pretend to know the final domain model. It creates a semantic staging layer from tables:

```text
table row → records
table cell → facts
issuing authority / nationality / callsign / aircraft model → entities where obvious
```

That is exactly what you want in dev-test. Once you see recurring `record_type` and `predicate` values, promote the important ones into stricter tables or views.
